...
import { sortOptionsForDisplay } from './pillStyle'

/** Always a dropdown checklist, regardless of option count — unlike
 * `SingleSelectControl`, which keeps pills for small option sets. A native
 * `<select multiple>` isn't a good fit here (clunky listbox, non-obvious
 * ctrl/cmd-click), so this opens a checklist popover from a single-line
 * trigger button instead. Kept consistent across all multi-selects rather
 * than switching to pills below a threshold, so the interaction pattern
 * doesn't change question to question. */
